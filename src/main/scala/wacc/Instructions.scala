...
object Instructions {
  ...
  case class CmpInstr(srcReg: Register, opr: Operand, shifter: Option[Shifter] = None)
      extends ExprInstr

  sealed trait MemoryInstr extends Instruction
  case class StoreInstr(srcReg: Register, destLoc: Operand, writeBack: Boolean = false)
      extends MemoryInstr
  case class StoreByteInstr(srcReg: Register, destLoc: Operand, writeBack: Boolean = false)
      extends MemoryInstr
  case class LoadInstr(dest: Register, srcLoc: Operand, writeBack: Boolean = false)
      extends MemoryInstr
  case class LoadSignedByteInstr(dest: Register, srcLoc: Operand, writeBack: Boolean = false)
      extends MemoryInstr
