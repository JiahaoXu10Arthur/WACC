...
class ParserExprTest extends AnyFlatSpec {
  ...
  "identifier" should "be parsed" in {
    ...
    Parser.exprParse("9").getOrElse("Failed") shouldBe "Failed"
  }
  ...
  "unary operator <expr>" should "be parsed" in {
    ...
    Parser.exprParse("chr 97").get shouldBe Ord(IntLit(97))
  }
  ...
  "<expr> binary operator <expr>" should "be parsed" in {
    ...
    Parser.exprParse("3<5").get shouldBe Lt(IntLit(3),IntLit(5))
    Parser.exprParse("3<=5").get shouldBe Lte(IntLit(3),IntLit(5))
    Parser.exprParse("5==5").get shouldBe Eq(IntLit(5),IntLit(5))
    Parser.exprParse("3!=5").get shouldBe Neq(IntLit(3),IntLit(5))
